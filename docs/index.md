...
#### 2022/05/13頃から録音が正常に完了しない（途中で終了やノイズが入る）障害が発生しているようです。ffmpegをバージョン4.2.1以前にすることで回避できるようです。ffmpegの4.2.1以前のバージョンは[こちらから入手できます。](https://ffbinaries.com/downloads#version_4.2.1)ご利用のOSにあったffmpegをダウンロードし、アプリ(CS-English/CS-Mult)と同じフォルダ内にあるffmpegと置き換えてください。
